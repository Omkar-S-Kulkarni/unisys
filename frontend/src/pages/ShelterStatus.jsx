...
export default function ShelterStatus() {
  ...
  const [liveShelters, setLiveShelters] = useState(cityData.shelters);
  const [liveZones, setLiveZones] = useState(cityData.zones);
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.city_model) {
        setLiveShelters(data.city_model.shelters || cityData.shelters);
        setLiveZones(data.city_model.zones || cityData.zones);
      }
    }
  }, [data]);
